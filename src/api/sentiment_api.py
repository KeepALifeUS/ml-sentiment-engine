"""
FastAPI Sentiment Analysis API для ML-Framework ML Sentiment Engine

Enterprise-grade API с Context7 patterns, rate limiting и async support.
"""
...
class SentimentRequest(BaseModel):
    """Запрос на анализ sentiment"""
    text: str = Field(..., min_length=1, max_length=10000, description="Текст для анализа")
    source: str = Field(default="api", description="Источник данных")
    model: str = Field(default="ensemble", description="Модель для анализа")
...
class BatchSentimentRequest(BaseModel):
    """Batch запрос на анализ sentiment"""
    texts: List[str] = Field(..., max_items=100, description="Список текстов")
    source: str = Field(default="api", description="Источник данных")
    model: str = Field(default="ensemble", description="Модель для анализа")
...
class DataSourceRequest(BaseModel):
    ...
    symbol: Optional[str] = Field(None, description="Символ криптовалюты")
    limit: int = Field(default=100, ge=1, le=1000, description="Лимит результатов")
    hours_back: int = Field(default=24, ge=1, le=168, description="Период в часах")
...
class SentimentAPI:
    """
    Enterprise-grade FastAPI приложение для sentiment analysis
    """
    ...
    def _setup_routes(self):
        ...
        async def analyze_sentiment(request: SentimentRequest):
            """Анализ sentiment для одного текста"""
