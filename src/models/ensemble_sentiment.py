"""
Ensemble Sentiment Model для ML-Framework ML Sentiment Engine
...
class EnsembleSentimentModel:
    ...
    async def initialize(self):
        """Инициализация всех constituent models"""
        try:
            # Инициализация transformer ensemble
    ...
    async def predict(
        ...
        """
        Ensemble prediction с адаптивным взвешиванием
        
        Args:
            text: Текст для анализа
            source: Источник данных для адаптации
            use_adaptive_weighting: Использовать адаптивные веса
    ...
    async def _get_model_predictions(
        ...
        """
        Получение предсказаний от всех моделей с error handling
        ...
        Returns:
            Dict[str, SentimentScore]: Результаты от каждой модели
        """
        results = {}
        ...
        try:
            ...
            # Обновление performance history
    ...
    def _adaptive_ensemble(
        ...
        """
        Adaptive ensemble на основе performance и confidence
        
        Args:
            model_results: Результаты от моделей
            source: Источник данных
            
        Returns:
            SentimentScore: Адаптивный ensemble результат
        """
        # Базовые веса для источника
        base_weights = self._get_source_weights(source)
        
        # Адаптация весов на основе confidence
        adaptive_weights = {}
        total_confidence = 0
        
        for model_name, result in model_results.items():
            if result.confidence > self.min_confidence_threshold:
                # Вес = базовый вес * confidence * performance_factor
        ...
        if sum(adaptive_weights.values()) > 0:
            ...
            # Fallback к равным весам если все confidence низкие
            adaptive_weights = {k: 1.0 / len(model_results) for k in model_results.keys()}
        
        # Вычисление ensemble sentiment
    ...
    def _weighted_ensemble(
        ...
        Returns:
            SentimentScore: Weighted ensemble результат
        """
    ...
    def _get_source_weights(self, source: str) -> Dict[str, float]:
        """
        Получение весов в зависимости от источника
        
        Args:
            source: Источник данных
            
        Returns:
            Dict[str, float]: Веса для каждой модели
        """
        if source in ["twitter", "reddit", "telegram", "discord"]:
            # Социальные медиа - VADER лучше работает
            return {
                "finbert": 0.3,
                "vader": 0.4,
                "textblob": 0.1,
    ...
    def _get_performance_factor(self, model_name: str) -> float:
        """
        Расчет performance factor для модели
        ...
        # Средняя confidence за последние предсказания
        avg_confidence = np.mean(history)
        
        # Конвертация в performance factor
        # Высокая confidence -> высокий factor
        return 0.5 + avg_confidence
    
    async def predict_batch(self, texts: List[str], source: str = "unknown") -> List[SentimentScore]:
        """
        Batch prediction для ensemble
        ...
        Returns:
            List[SentimentScore]: Результаты ensemble
        """
        results = []
        
        # Параллельная обработка небольшими батчами
        batch_size = 10
...
async def create_ensemble_model() -> EnsembleSentimentModel:
    """
    Factory для создания ensemble модели
    
    Returns:
        EnsembleSentimentModel: Инициализированная ensemble модель
    """
